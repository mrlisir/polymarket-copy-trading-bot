...
> 前置背景见：`docs/更新日志_V2.md`、`docs/更新日志_V1.md`。
...
## 六、代理与 RPC 分流：Polymarket 走代理，链上 RPC 强制直连

**文件**：`src/config/env.ts`、`src/utils/healthCheck.ts`

### 背景

- 启用 HTTP 代理后，Polymarket API 可访问，但链上 RPC 健康检查出现超时。
- 根因是进程级 `HTTP_PROXY/HTTPS_PROXY` 生效后，RPC 请求也被代理转发。

### 改动

- 在 `env.ts` 中保留代理开关能力，同时新增直连旁路逻辑：
  - 当 `HTTP_PROXY_ENABLED=true` 时，自动将 `RPC_URL` 对应主机加入 `NO_PROXY/no_proxy`；
  - 额外补充 `localhost`、`127.0.0.1` 到旁路列表，避免本地地址误走代理。
- 在 `healthCheck.ts` 中将 RPC 检查改为 `axios.post(...)` 并显式设置 `proxy: false`：
  - 对 RPC 健康检查做强制直连；
  - 避免被全局代理环境变量影响。

### 作用

- 实现“Polymarket 请求可代理、链上 RPC 保持直连”的并行网络策略。
- 降低代理环境下 `RPC check failed: timeout` 的概率，启动健康检查更稳定。

### 验证

- 执行 `npm run health-check`：
  - `RPC 节点: ✅ RPC endpoint responding`
